...
llm_moe_expert_ids llm_build_moe_expert_ids(
    ...
    if (arch == LLM_ARCH_GROVEMOE && n_expert != n_expert_total) {
        GGML_ASSERT(n_group_experts > 0);
        // TODO: Use scalar div instead when/if implemented
        ggml_tensor * f_sel = ggml_cast(ctx, selected, GGML_TYPE_F32);
        lookup = ggml_cast(ctx, ggml_scale(ctx, f_sel, 1.0f / float(n_group_experts)), GGML_TYPE_I32);
    }
    return { selected, lookup ? lookup : selected };
}
...
ggml_tensor * llm_graph_context::build_moe_ffn(
     ...
     llm_ffn_op_type   type_op,
         ...
         ggml_tensor * selected_experts_lookup_in) const {
...
ggml_tensor * llm_graph_context::build_moe_ffn(
    ...
    const llm_moe_expert_ids expert_ids = llm_build_moe_expert_ids(
            ctx0, arch, selected_experts, selected_experts_lookup_in,
            n_expert, hparams.n_expert, hparams.n_group_experts);
    selected_experts = expert_ids.routing;
    ggml_tensor * selected_experts_lookup = expert_ids.lookup;
    ...
    if (gate_up_exps) {
        ...
        ggml_tensor * gate_up = build_lora_mm_id(
                gate_up_exps, cur, selected_experts_lookup, up_exps_s, selected_experts); // [n_ff*2, n_expert_used, n_tokens]
        ...
        up = build_lora_mm_id(
                up_exps, cur, selected_experts_lookup, up_exps_s, selected_experts); // [n_ff, n_expert_used, n_tokens]
        cb(up, "ffn_moe_up", il);
        ...
        if (gate_exps) {
            cur = build_lora_mm_id(
                    gate_exps, cur, selected_experts_lookup, gate_exps_s, selected_experts); // [n_ff, n_expert_used, n_tokens]
            cb(cur, "ffn_moe_gate", il);
        } else {
            cur = up;
        }
        ...
        case LLM_FFN_SILU:
            if (gate_exps) {
                if (il >= 0) {
                    ...
                    constexpr float eps = 1e-6f;
                    if (limit > eps) {
                        if (arch == LLM_ARCH_MAPLE || arch == LLM_ARCH_DEEPSEEK4 || arch == LLM_ARCH_DEEPSEEK41 ||
    ...
    experts = build_lora_mm_id(
            down_exps, cur, selected_experts_lookup, down_exps_s, selected_experts); // [n_embd, n_expert_used, n_tokens]
