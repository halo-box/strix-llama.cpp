...
public:
    ...
    bool can_reuse(const llm_graph_params & params) override {
        ...
        // the block table is a graph input only on the inline path; with the pooled-key cache
        // it stays in host scratch and these two are never created
        ...
        if (pool_cells != nullptr) {
            const int64_t want = (int64_t) ratio *
                mctx->qsa_pool_n_recomp(ratio, (uint32_t) params.ubatch.n_tokens, (uint32_t) n_kv, QSA_N_PAD_KV);
