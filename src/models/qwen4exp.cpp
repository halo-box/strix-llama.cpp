...
ggml_tensor * llama_model_qwen4exp::graph::build_attn_qsa(
    ...
    // a mask with per-stream padding cannot be gathered per query; scan stays correct there
    const ggml_tensor * gather_mask = inp->get_kq_mask();
    ggml_tensor * cur = cparams.flash_attn && 4*n_tps*width < n_kv &&
            gather_mask->nb[3] == gather_mask->nb[1]*n_tps
