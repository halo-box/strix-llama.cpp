...
static __global__ void moe_weighted_reduction_f32(const float * __restrict__ experts,
    ...
    const int64_t col   = ((int64_t) blockIdx.y * blockDim.x + threadIdx.x) * values_per_thread;
    if (col >= n_embd) {
        return;
    }
    ...
    float          sum[values_per_thread];
#pragma unroll
    for (int value = 0; value < values_per_thread; ++value) {
        sum[value] = (experts[first_row * n_embd + col + value] * first_scale) * weights[first_row];
    }
...
#pragma unroll
        for (int value = 0; value < values_per_thread; ++value) {
            sum[value] += (experts[row * n_embd + col + value] * scale) * weights[row];
        }
    }
#pragma unroll
    for (int value = 0; value < values_per_thread; ++value) {
        dst[token * n_embd + col + value] = sum[value];
    }
}
...
static void launch_moe_weighted_reduction(const float * experts,
    ...
    if (n_embd % 4 == 0) {
        const dim3 blocks(n_tokens, (n_embd + 4 * threads - 1) / (4 * threads), 1);
        moe_weighted_reduction_f32<4>
