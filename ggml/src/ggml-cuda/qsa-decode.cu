...
#pragma unroll
    ...
    float * dst = partial + ((size_t(query)*nh + head)*splits + split)*258;
...
static __global__ __launch_bounds__(256) void qsa_decode_merge(
    ...
    const float * src = partial + size_t(row)*splits*258;
    float maximum = -INFINITY;
    ...
    out[size_t(row)*256 + d] = sum > 0.0f ? value/sum : 0.0f;
}
...
static __global__ void qsa_decode_unique_ids(const char * input, size_t stride, int * output, int count, int keys) {
    __shared__ int seen[8192];
    const int tid=threadIdx.x, row=blockIdx.x;
    for (int i=tid;i<8192;i+=blockDim.x) seen[i]=-1;
    __syncthreads();
    const auto * src=reinterpret_cast<const int *>(input+size_t(row)*stride);
    int * dst=output+size_t(row)*count;
    for (int j=tid;j<count;j+=blockDim.x) {
        const int key=src[j];
        dst[j]=-1;
        if (key<0 || key>=keys) continue;
        unsigned slot=(unsigned(key)*2654435761u)&8191u;
        while (true) {
            const int old=atomicCAS(seen+slot,-1,key);
            if (old==-1) { dst[j]=key; break; }
            if (old==key) break;
            slot=(slot+1)&8191u;
        }
    }
...
void ggml_cuda_flash_attn_ext_qsa_decode(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    const auto * q = dst->src[0], * k = dst->src[1], * v = dst->src[2], * m = dst->src[3], * ids = dst->src[5];
    ggml_cuda_pool_alloc<int> unique_ids(ctx.pool(), size_t(ids->ne[0])*q->ne[1]);
    qsa_decode_unique_ids<<<q->ne[1],256,0,ctx.stream()>>>((const char *)ids->data,ids->nb[1],unique_ids.get(),ids->ne[0],k->ne[1]);
